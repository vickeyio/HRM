  ...
  <form @submit.prevent="handleSubmit">
    ...
    <div class="mb-3">
      <label class="form-label text-dark fw-medium fs-13 mb-1">
        Group Name <span class="text-danger" v-if="!readonly">*</span>
      </label>
      <input
        type="text"
        v-model="localForm.name"
        class="form-control"
        :class="{ 'is-invalid': !!fieldErrors?.name || !!fieldErrors?.group_name }"
        placeholder="e.g. Executive Staff or HR Admins"
        :disabled="readonly || isLoading"
        required
      />
      <div v-if="fieldErrors?.name || fieldErrors?.group_name" class="invalid-feedback">
        {{ fieldErrors?.name || fieldErrors?.group_name }}
      </div>
    </div>
...
const localForm = ref({
  name: props.formData.name || props.formData.group_name || '',
...
watch(
  ...
  (newVal) => {
    localForm.value = {
      name: newVal?.name || newVal?.group_name || '',
