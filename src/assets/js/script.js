	...
	// setTimeout(function () {
		setTimeout(function () {
			$("#global-loader").fadeOut({
				duration: 200, // Increased slightly for smooth browser rendering
				easing: "linear", 
				complete: function() {
					$(this).hide().remove(); // hide() first ensures it disappears in Firefox
				}
			});
	...
	$(document).ready(function() {
		if($('.yearpicker').length > 0 ){
			$('.yearpicker').datetimepicker({
				viewMode: 'months', 
				// 'MMM' shows short month (Dec, Nov, Jan)
				format: 'MMM YYYY', 
				useCurrent: true,
				icons: {
